...
venueCounts = FOREACH (GROUP venueMentions BY (venuesReduced::Name, venuesReduced::Type, tweetsReduced::Year)) GENERATE group, COUNT($1) as counter;

flattenVenues = foreach venueCounts generate flatten(group), counter;

STORE flattenVenues INTO 'VenueYearCat' USING org.apache.pig.piggybank.storage.DBStorage('com.mysql.jdbc.Driver','jdbc:mysql://storo:3306/teamn', 'teamn', '8553mkpw','INSERT INTO VenueYearCat (Name, Category, Year, Hits) VALUES (?, ?, ?, ?)');
